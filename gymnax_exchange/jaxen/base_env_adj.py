    ...
    def __init__(self, alphatradePath):
        ...
        def load_LOBSTER(sliceTimeWindow, stepLines, messagePath, orderbookPath, start_time, end_time):
            ...
            def preProcessingMassegeOB(message, orderbook):
                ...
                # Change: filter orderbook based on valid indices
                filtered_orderbook = orderbook.iloc[valid_index, :].reset_index(drop=True)
                return message, filtered_orderbook
            ...
            def sliceWithoutOverlap(message, orderbook):
                ...
                def splitMessage(message, orderbook):
                    ...
                    for i in range(len(indices) - 1):
                        ...
                        print(f"Orderbook Slice {i}: {orderbook.iloc[index_s, :].values}")  # Debugging line

                    # Last sliced part from last index to end_time
                    start_index = indices[-2]
                    end_index = indices[-1]
                    index_s, index_e = message[(message['time'] >= start_index) & (message['time'] < end_time)].index[[0, -1]].tolist()
                    ...
                    init_OBs.append(orderbook.iloc[index_s, :])
                    print(f"Orderbook Slice (Last): {orderbook.iloc[index_s, :].values}")  # Debugging line
                ...
                slicedCubes = [sliced2cube(sliced) for sliced in sliced_parts]
                slicedCubes_withOB = list(zip(slicedCubes, init_OBs))
    ...
    return Cubes_withOB, max_steps_in_episode_arr
        ...
        def get_state(message_data, book_data, max_steps_in_episode):
            ...
            def get_initial_orders(book_data, time):
                ...
                initid=-9000
                '''
                'Adjusted by Mike 10.06'
                original: data=jnp.array(book_data).reshape(int(10*2),2)
                '''
